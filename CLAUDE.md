...
- The two overlay cards are collapsible drawers (`toggleDrawer`), remembered per viewer in `localStorage` and collapsed by default under 640px. The legend opens sideways; `#title` carries `.drawer-stacked` and opens downward, since its content reads as a column. Both collapse to just their arrow — the header text is width-collapsed too, otherwise a collapsed card is still a block sitting on the map, which defeats the point.
