...
### Adding real ground sensors

The CPCB operates reference monitors in the region whose data is available through [OpenAQ](https://openaq.org) and [WAQI](https://aqicn.org/data-platform/token/) — both need a free API key, which is why they aren't the default here. To use one, add the key as a repository secret, fetch it alongside the model data in `src/fetch-aqi.js`, and mark those stations so the dashboard can distinguish measured from modelled values.
